...
def test_about_page(client: client):
    """Test the about page of the site"""
    response = client.get("/about")
    assert response.status_code == 200


def test_account_page(client: client):
    """"""
    response = client.get("/account")
    assert response.status_code == 302


def test_users_page(client: client):
    """"""
    response = client.get("/users")
    assert response.status_code == 302


def test_todo_page(client: client):
    """"""
    response = client.get("/todo/")
    assert response.status_code == 302


def test_not_found_page(client: client):
